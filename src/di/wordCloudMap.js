...
const wordCloudMap = (inputString) => {
  ...
  for(let i=0; i<splitStringAtSpaces.length; i++){
    ...
    // remove it
    if( !alphabet.has(splitStringAtSpaces[i].charAt(0)) ){
      splitStringAtSpaces[i] = splitStringAtSpaces[i].slice(1)
    } else if( !alphabet.has(splitStringAtSpaces[i].charAt(splitStringAtSpaces[i].length - 1)) ){
...
console.log(wordCloudMap("I am a. sample! input string, do you love me?"))
